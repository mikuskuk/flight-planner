...
public class FlightInMemoryService implements FlightService {
    ...
    public synchronized Flight addFlight(AddFlightRequest request) {
        ...
        List<Flight> existingFlights = flightInMemoryRepository.listFlights();
        ...
        flightInMemoryRepository.saveFlight(flight);
        return flight;
    }
    ...
    public List<Airport> searchAirport(String search) {
        List<Airport> airportsFrom = flightInMemoryRepository.listFlights().stream()
                .map(Flight::getFrom)
                .toList()
        ...
        List<Airport> airportsTo = flightInMemoryRepository.listFlights().stream()
                .map(Flight::getTo)
                .toList()
                .stream().filter(airport -> airport.getAirport().toLowerCase().contains(search.toLowerCase().trim()) ||
                        airport.getCity().toLowerCase().contains(search.toLowerCase().trim()) ||
    ...
    public PageResult<Flight> searchFlights(SearchFlightsRequest request) throws IllegalArgumentException{
        ...
        List<Flight> flights = flightInMemoryRepository.listFlights().stream()
    ...
    public Flight fetchFlight(long id) throws IllegalArgumentException{
        try {
            return flightInMemoryRepository.listFlights().stream()
